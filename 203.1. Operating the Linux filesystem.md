...
| &lt;mount point&gt; | /media/floppy or UUID | Where do we wana mount device/partition |
...
in fstab we can use UUID \(Universally Unique Identifier\) instead of Device abstract name from /dev/ directory. This way we reduce the mount fails because  HAL might change the name as time passes.

And Finnally lets make the swap partition that we have made permanent by editing fstab file :
