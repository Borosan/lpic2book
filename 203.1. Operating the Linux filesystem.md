...
-t means what type of file system is going to be mounted, /dev/sdb1 is mount device and /mnt/my10ghdd is mount point.to mount iso:
...
Every thing seems  right but these our mounted devices are  not persistent and wouldn't be accessible after reboot so far.To make a persistent mount we should use fstab. fstab is very similar to mtab but they are not related. fstab is more easier
...
| &lt;type&gt; | ext4 , sw, | Type of File system |
...
Before explaining mount options in fstab, lets talk about sync/async concept
...
| fstab mount option | Description |
| :--- | :--- |
| ro | Read only |
| rw | Read Write |
| exec/noexec | permit/prevent execution of binaries |
| auto | File system is mounted automatically after boot process, there is no need to use mount -a |
| noauto | File system is not mounted automatically after boot, we can use mount -a option after reboot |
| suid/nosuid | permit/block the operation of suid/guid bits |
| user | permit any user to mount file system. equivalent to noexec, nosuid,nodev \[unless overridden\] |
| nouser | Only root can mount file system, this is default setting |
| dev/nodev | Interpret/Do not interpret character or block special devices on the file system |
| sysnc/async | I/O to the file system is done \(a\)synchronously |
