...
NVM Express \(NVMe\) devices are flash memory chips connected to a system via the PCI-E bus.Linux has NVMe driver which is natively included in the kernel since version 3.3. NVMe devices should show up under /dev/nvme\*
...
To keep it simple we use CentOS 6 server \(target\) with additional 10 gig Disk Drive and then get Our Ubuntu client \(initiator\) connected to it. First We need to add EPEL repository:
...
and we are done. Now we can format mount the partition or we can add it to fstab.
