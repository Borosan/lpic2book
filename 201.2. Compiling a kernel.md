...
As we have seen kernel needs RAM to be populated with some parameters and values.Also we talked about this fact that initramfs is used inside initrd inorder to populate RAM.In old days each distro tries to implement its own taste of initramfs. It made hard for users and developers to work with each distro. dracut is a tool which has tried to standardize whole process.

dracut uses udev deamon events \(uevents\) and based on them create general purpose initramfs.  All we need to know is that dracut is a program wich is used behind kernel installation process. it takes our new compiled kernel and current system setting and put required files in right places and configure grub and finally let us to boot with our new kernel. 
