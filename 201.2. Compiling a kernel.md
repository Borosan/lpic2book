...
As we have seen kernel needs RAM to be populated with some drivers and modules.In old days each distro tries to implement its own taste of initramfs.initrd. It made hard for users and developers to work with each distro. dracut is a tool which has tried to standardize whole process.

dracut uses udev deamon events \(uevents\) and based on them create general purpose initramfs.  All we need to know is that dracut is a program wich is used behind kernel installation process to create initramfs. It consider curent kernel and settings anf generate required initramfs/initrd base on them and put initramfs/initrd on the right place and modified grub.
