...
Base on our distro we might need to install xfsprogs to use XFS tools:

```
root@server1:~# apt install xfsprogs
...
| xfs\_check | Check XFS File System problems with no verbosity, use -v or "xfs\_repair -n" instead |

```
root@server1:~# mount /dev/sdc1 /mnt/mysecond10ghard/
root@server1:~# xfs_info /mnt/mysecond10ghard/
meta-data=/dev/sdc1              isize=512    agcount=4, agsize=655296 blks
         =                       sectsz=512   attr=2, projid32bit=1
...
root@server1:~# umount /mnt/mysecond10ghard 

root@server1:~# xfs_repair -n /dev/sdc1
...
root@server1:~# xfs_repair  /dev/sdc1
Phase 1 - find and verify superblock...
Phase 2 - using internal log
        - zero log...
        - scan filesystem freespace and inode maps...
